...
void get_pixelarr_2bpp(FILE *bmp_in, Image_2bpp *Image, DWORD bfOffset, LONG biHeight, LONG biWidth)
{
	...
	uint32_t row_size = ceil((biWidth * 2) / 32.0) * 4;


	// allocating memory for the pixel array and writing to it
	Image->pixel_arr = (Pixel_2bpp**) malloc(Image->height * sizeof(Pixel_2bpp*));
	...
	{
		Image->pixel_arr[i] = (Pixel_2bpp*) malloc(Image->width * sizeof(Pixel_2bpp));
		...
		// iterating over each byte and then each 2 bit chunk in it, breaking when last chunk is reached
		...
		{
			for (int bit_chunk = 0; bit_chunk < 4; bit_chunk++) // we have 4 chunks of 2 bits in a byte
			{
				// shifting and masking
				(Image->pixel_arr[i][byte * 4 + bit_chunk]).bits = (buffer[byte] >> (6 - bit_chunk * 2)) & 0x3; // 0x3 = 0b0011
				counter++;
				if (counter == biWidth) break; // biWidth is equal to the amount of 2 bit chunks
			}
	...
	if (biHeight < 0)
	{
		if (rotate_2bpp(Image) != 0 || rotate_2bpp(Image) != 0)
		{
...
void flip_2bpp(Image_2bpp *Image)
{
	for (int i = 0; i < Image->height; i++)
	{
		for  (int j = 0; j < Image->width / 2; j++)
		{
			...
			Pixel_2bpp temp = Image->pixel_arr[i][j];
...
int rotate_2bpp(Image_2bpp *Image)
{
	...
	Pixel_2bpp **temp_arr = (Pixel_2bpp**) malloc(temp_height * sizeof(Pixel_2bpp*));
	if (temp_arr == NULL)
	{
	...
	{
		temp_arr[i] = (Pixel_2bpp*) malloc(temp_width * sizeof(Pixel_2bpp));
		if (temp_arr[i] == NULL)
		{
			perror("Error allocatng memory for temporary array in rotate function");
			// freeing all previous rows
...
void write_2bpp(char *output_path, Image_2bpp *Image, BITMAPFILEHEADER *header, BITMAPINFOHEADER *dheader)
{
	...
	uint32_t row_size = ceil((dheader->biWidth * 2) / 32.0) * 4;
	...
	BYTE empty_bytes = row_size - ceil((dheader->biWidth * 2) / 8.0);
	...
	for (int i = 0; i < Image->height; i++)
	{
		...
		{
			byte |= Image->pixel_arr[i][j].bits << (6 - chunk_counter * 2);
			chunk_counter++;
